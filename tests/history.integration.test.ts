...
import { attemptActivity, attemptGroups, attemptItems, attempts, exerciseItems, exerciseRevisions, exercises, l2Sessions, users } from "@/db/schema";
...
it("repeats a frozen batch as a fresh attempt without modifying answers or unique coverage", async () => {
  ...
  for (const type of ["R1", "R3", "L2", "W1", "W2"] as const) {
    ...
    const { data: { id, typeCode } } = await repeated.json();
    expect(id).not.toBe(source.id);
    expect(typeCode).toBe(type);
    ...
    if (type === "L2") expect(await db.select().from(l2Sessions).where(eq(l2Sessions.attemptId, id))).toHaveLength(1);
