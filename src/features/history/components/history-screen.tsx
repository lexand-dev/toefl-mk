...
function HistoryList() {
  ...
  const repeat = useMutation({ mutationFn: async (id: string) => dataOrThrow<{ id: string }>(await historyApi.api.history.attempts[":id"].repeat.$post({ param: { id } })), onSuccess: ({ id }) => router.push(`/app/practice/${id}`) });
  return <article><h1>Historial y actividad</h1>
    ...
    {attempts.isPending ? <p>Cargando intentos…</p> : attempts.isError ? <p className="error" role="alert">{attempts.error.message}</p> : !attempts.data.length ? <p>No hay intentos para estos filtros.</p> :
      <ul>{attempts.data.map((attempt) => <li key={attempt.id}>
        ...
        <nav><Link href={`/app/practice/${attempt.id}`}>{attempt.status === "submitted" ? "Revisar" : "Continuar"}</Link>
