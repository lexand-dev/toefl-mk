...
type ErrorResult = { error: string; status: 404 | 409 | 422 };
const fail = (error: string, status: ErrorResult["status"]): ErrorResult => ({ error, status });
export const groupsSchema = z.union([z.literal(10), z.literal(20)]);
export const selectionSchema = z.object({ groups: groupsSchema, timerMode: z.enum(["count_up", "count_down"]) }).strict();
export const responseSchema = z.object({ version: z.number().int().nonnegative(), response: z.object({ tokenIds: z.array(z.string().min(1)) }).strict() }).strict();
export const rules = { version: "W1-1", secondsPerGroup: 60, navigation: "bidirectional" } as const;
...
async function candidates(connection: Connection) {
  const rows = await connection.select({ id: exerciseRevisions.id, exerciseId: exercises.id, revisionNumber: exerciseRevisions.revisionNumber })
    ...
    .where(and(eq(exercises.typeCode, "W1"), eq(exerciseRevisions.status, "published")))
...
export async function availability(groups: 10 | 20) {
  const available = await candidates(db);
  return { typeCode: "W1" as const, requestedGroups: groups, materialCount: Math.min(groups, available.length), itemCount: available.slice(0, groups).reduce((sum, row) => sum + row.itemCount, 0), availableMaterials: available.length, shortage: Math.max(0, groups - available.length), rules };
}
...
export async function openAttempts(userId: string) {
  return db.select({ id: attempts.id, status: attempts.status, createdAt: attempts.createdAt })
    .from(attempts).where(and(eq(attempts.userId, userId), eq(attempts.typeCode, "W1"), inArray(attempts.status, ["prepared", "in_progress"]))).orderBy(desc(attempts.createdAt));
}

export async function create(userId: string, groups: 10 | 20, timerMode: "count_up" | "count_down") {
  return db.transaction(async (tx) => {
    ...
    await tx.insert(attempts).values({ id, userId, typeCode: "W1", requestedGroups: groups, timerMode, rulesSnapshot: rules });
    let position = 1;
...
async function locked(tx: Tx, id: string, userId: string) {
  const [attempt] = await tx.select().from(attempts).where(and(eq(attempts.id, id), eq(attempts.userId, userId), eq(attempts.typeCode, "W1"))).for("update");
  return attempt;
}

function sequences(answers: unknown): string[][] {
  if (!Array.isArray(answers)) throw new Error("Clave W1 inválida");
  return answers.every((value) => typeof value === "string") ? [answers as string[]] : answers as string[][];
}

async function close(tx: Tx, attempt: typeof attempts.$inferSelect, time: Date) {
  if (attempt.status === "submitted") return;
  const rows = await tx.select({ id: attemptItems.id, response: attemptItems.responseJson, revisionId: attemptItems.revisionId, itemId: attemptItems.itemId, pointsPossible: attemptItems.pointsPossible, prompt: exerciseItems.publicPrompt })
  ...
  for (const row of rows) {
    ...
    const ids = (row.response as { tokenIds: string[] } | null)?.tokenIds ?? [];
    const complete = ids.length === promptSchemas.W1.parse(row.prompt).tokens.length;
    const correct = complete && sequences(key.acceptedAnswers).some((sequence) => sequence.length === ids.length && sequence.every((token, index) => token === ids[index]));
    possible += Number(row.pointsPossible);
    if (correct) awarded += Number(row.pointsPossible);
    await tx.update(attemptItems).set({ outcome: !complete ? "omitted" : correct ? "correct" : "incorrect", pointsAwarded: correct ? row.pointsPossible : "0" }).where(eq(attemptItems.id, row.id));
  }
...
export async function detail(id: string, userId: string) {
  return db.transaction(async (tx) => {
    ...
    return { id, typeCode: "W1" as const, status, timerMode: attempt.timerMode, rules: attempt.rulesSnapshot, currentPosition: attempt.currentPosition, serverTime: time.toISOString(), startedAt: attempt.startedAt?.toISOString() ?? null, deadlineAt: attempt.deadlineAt?.toISOString() ?? null, submittedAt: status === "submitted" ? (attempt.submittedAt ?? time).toISOString() : null,
      ...
      groups: groups.map((group) => ({ id: group.id, ordinal: group.ordinal, revisionId: group.revisionId, content: contentSchemas.W1.parse(group.content) })),
      items: items.map((item) => ({ id: item.id, groupId: item.groupId, position: item.position, prompt: promptSchemas.W1.parse(item.prompt), response: item.response as { tokenIds: string[] } | null, version: item.version, savedAt: item.savedAt?.toISOString() ?? null, ...(status === "submitted" ? { outcome: item.outcome, pointsPossible: Number(item.pointsPossible), pointsAwarded: Number(item.pointsAwarded), acceptedSequences: sequences(keys.find((key) => key.itemId === item.itemId && key.revisionId === item.revisionId)?.acceptedAnswers), explanation: keys.find((key) => key.itemId === item.itemId && key.revisionId === item.revisionId)?.explanation } : {}) })),
    };
  });
}
...
export async function save(id: string, userId: string, itemId: string, version: number, tokenIds: string[]) {
  return db.transaction(async (tx) => {
    const attempt = await locked(tx, id, userId);
    if (!attempt) return fail("No encontrado", 404);
    const time = await now(tx);
    if (await reconcile(tx, attempt, time) !== "in_progress") return fail("El intento no admite respuestas", 409);
    const [item] = await tx.select({ version: attemptItems.responseVersion, prompt: exerciseItems.publicPrompt }).from(attemptItems)
      .innerJoin(exerciseItems, and(eq(exerciseItems.id, attemptItems.itemId), eq(exerciseItems.revisionId, attemptItems.revisionId)))
    ...
    const tokens = promptSchemas.W1.parse(item.prompt).tokens;
    if (tokenIds.length > tokens.length || new Set(tokenIds).size !== tokenIds.length || tokenIds.some((tokenId) => !tokens.some((token) => token.id === tokenId))) return fail("Secuencia de fichas inválida", 422);
    await tx.update(attemptItems).set({ responseJson: { tokenIds }, responseVersion: version + 1, savedAt: time }).where(eq(attemptItems.id, itemId));
...
export async function reconcileDeadline(id: string, scheduler: DeadlineScheduler = triggerDeadlineScheduler) {
  await db.transaction(async (tx) => {
    const [attempt] = await tx.select().from(attempts).where(and(eq(attempts.id, id), eq(attempts.typeCode, "W1"))).for("update");
    if (!attempt) throw new Error(`W1 attempt ${id} does not exist`);
